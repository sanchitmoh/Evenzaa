...
export default function EventForm() {
  ...
  // Add debug logging
  console.log('=== EventForm Debug Info ===');
  ...
  useEffect(() => {
    if (!isEditMode) {
      ...
      console.log('Setting initial category:', category, 'for event type:', eventType);
    ...
    if (isEditMode) {
      ...
      axios.get(endpoint, {
        ...
        .catch(err => {
          ...
          setError(`Failed to fetch ${eventType} details: ${err.response?.data?.message || err.message}`);
          setLoading(false);
        });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.status === 401 
        ...
        : `Failed to save ${eventType}: ${err.response?.data?.message || err.message || 'Unknown error'}`;
